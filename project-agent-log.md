...
- Summary:
  ...
  - Optimized the batch reporting mechanism to update the `fragmentation_summary.csv` incrementally. Instead of waiting for the entire batch to complete, the script now appends results to the CSV immediately as each file finishes processing, ensuring data persistence even if the process is interrupted.
